...
"""
Code to track a linear path 
From the initial position of the end-effector, we make a straight line path to a desired point
ros2 run painter line_trajectory_controller.py
"""
...
class PositionControllerNode(Node):
    ...
    def __init__(self):
        ...
        self.x_des = np.linspace(transform[0, 3], 0.5, self.num_points)
        self.y_des = np.linspace(transform[1, 3], 0.5, self.num_points)
        self.z_des = np.linspace(transform[2, 3], 0.5, self.num_points)
